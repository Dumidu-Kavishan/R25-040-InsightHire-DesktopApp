"""
EDUGuard Backend Routes

This module contains all the route definitions for the EDUGuard backend application.
It is designed to be imported by the main.py file, which serves as the entry point.
"""
...
def register_app_routes(app):
    ...
    def register():
        ...
        try:
            ...
            logger.info(f"[SUCCESS] User registered successfully: {user.uid}")
    ...
    def login():
        ...
        try:
            ...
            if not user_data:
                ...
                db_manager.create_user_profile(user_data)
                logger.info(f"[SUCCESS] Created new user profile for: {user_id}")
            else:
                # Update last login time
                update_data = {'lastLogin': datetime.now().isoformat()}
                db_manager.update_user_profile(update_data)
                user_data.update(update_data)
                logger.info(f"[SUCCESS] User login updated: {user_id}")
...
# This section is kept for backward compatibility - allows running app.py directly
if __name__ == '__main__':
    ...
    logger.warning("Running app.py directly is deprecated. Please use main.py instead.")
